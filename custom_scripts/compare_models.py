...
weights_1 = r"C:\Users\v.stecko\Desktop\YOLOv5 Project\server\yolov5\runs\yolov5m_6classes_BIG_AUGMENTATED_ver5.pt"
version_1 = 'v5'
weights_2 = r"C:\Users\v.stecko\Desktop\YOLOv5 Project\server\yolov5\runs\yolov5m_6classes_BIG_AUGMENTATED_ver6.pt"
version_2 = 'v5'

image_path = r"D:\YOLOv5 DATASET\STEP 10 (New Mozyr)\No labled\2025-04-30-12-13-40-381664_clear.jpg"
...
model_2 = load_model(version_2, project_path, weights_2)

img = (cv2.imread(image_path, cv2.COLORMAP_INFERNO))
img_resize = cv2.resize(img, (640, 640))
norm_image = img.astype(np.uint8)

detected_img_1 = processing(model=model_1, version=version_1, img=norm_image.copy())
detected_img_2 = processing(model=model_2, version=version_2, img=norm_image.copy())


cv2.imshow(f"Model-1 result (version {version_1})", detected_img_1)
cv2.imshow(f"Model-2 result (version {version_2})", detected_img_2)

# combined = np.hstack((detected_img_1, detected_img_2))  # по горизонтали
# cv2.imshow("Comparison: Model 1 vs Model 2", combined)
cv2.waitKey(0)
