...
def main(ip, ports):
    ...
    for port in ports:
        try:
            cl = Client((ip, int(port)), config[int(port)], logger.bind(port=port))
        except KeyError:
            cl = Client((ip, int(port)), config['default'], logger.bind(port=port))
            logger.warning(f'Unknown port {port}! Loaded default configuration.')
        cl.start()
        processes.append(cl)
