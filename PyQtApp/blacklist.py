                ...
                try:
                    ...
                    while True:
                        try:
                            ...
                            self.logger.trace(f'Receiving data...')
                        except Exception as e:
                            self.logger.error(e)
                        if img_arr is not None:
                            try:
                                clear_img, df_result, detected_img = self.nn.processing_for_grpc(img_arr)
                                self.logger.trace(f'df_result= {df_result}')
                            except Exception as e:
                                self.logger.error(e)
                            ...
                            if self.data_q is not None:
                                try:
                                    ...
                                    self.logger.trace(f'Accumulate and make decision...')
                                except Exception as e:
                                    self.logger.error(e)
                                ...
                                try:
                                    ...
                                    try:
                                        self.data_q.put({'name': self.name,
                                                         ...
                                                         'spectrum': spectrum},
                                                        timeout=10)
                                        self.logger.trace(f'Put data to queue...')
                                    except Exception as e:
                                        self.logger.error(e)
